...
//! The basic `Controller` trait has no dependency on Entity or ORM.
//! For ORM-backed CRUD defaults, enable the `orm` feature.

use crate::http::{Request, Response};

/// Basic controller trait — maps a Request to a Response.
///
/// No dependency on Entity, Service, or ORM.
pub trait Controller: std::fmt::Debug + Send + Sync + 'static {
    fn handle(&self, req: &Request) -> Response;
}

/// ORM-backed defaults: DefaultController + CRUD actions.
///
/// Requires `orm` feature — provides `index`, `show`, `store`, `update`, `destroy`.
#[cfg(feature = "orm")]
    ...
    pub trait HandlesCrud<M: Entity + serde::Serialize + 'static>: std::fmt::Debug + Send + Sync {
        ...
        fn index(&self, req: Request) -> Response {
            ...
            let mut res = match items {
                ...
                Err(e) => {
                    let mut r = Response::html(&e.to_string());
                    set_status(&mut r, StatusCode::SERVER_ERROR);
                    r
                }
            };
            self.after(&req, &mut res, "index");
            res
        }
        ...
        fn show(&self, req: Request) -> Response {
            ...
            let mut res = match self.service().find(id) {
                ...
                Ok(None) => {
                    let mut r = Response::html("Not found");
                    set_status(&mut r, StatusCode::NOT_FOUND);
                    r
                }
                Err(e) => {
                    let mut r = Response::html(&e.to_string());
        ...
        fn store(&self, req: Request) -> Response {
            ...
            let owned_data = match parse_json_body::<serde_json::Value>(&req) {
                ...
                Ok(_) => {
                    let mut r = Response::html("Expected JSON object");
                    set_status(&mut r, StatusCode::BAD_REQUEST);
                    return r;
                }
                Err(e) => {
                    let mut r = Response::html(&e);
                    set_status(&mut r, StatusCode::BAD_REQUEST);
                    return r;
                }
            };
            ...
            let mut res = match self.service().create(data) {
                Ok(id) => {
                    ...
                    set_status(&mut r, StatusCode::CREATED);
                    r
                }
                Err(e) => {
                    let mut r = Response::html(&e.to_string());
        ...
        fn update(&self, req: Request) -> Response {
            ...
            let data = owned_to_ref(&owned_data);
            let mut res = match self.service().update(id, data) {
                Ok(affected) => {
                    let body = serde_json::json!({ "affected": affected }).to_string();
                    Response::text(&body).with_header("content-type", "application/json")
                }
                Err(e) => {
                    let mut r = Response::html(&e.to_string());
        ...
        fn destroy(&self, req: Request) -> Response {
            ...
            self.after(&req, &mut res, "destroy");
            res
        }
    }
        ...
        obj.iter().map(|(k, v)| {
            let val = match v {
                ...
                serde_json::Value::Number(n) => n.as_i64().map(Value::Int)
                    ...
                    .unwrap_or(Value::Text(n.to_string())),
