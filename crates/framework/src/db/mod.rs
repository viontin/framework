...
#[derive(Debug, Clone)]
pub enum Value { Null, Int(i64), Float(f64), Text(String), Bool(bool), Blob(Vec<u8>), }
...
impl Row {
    ...
    pub fn int(&self, col: &str) -> Option<i64> { self.columns.get(col).and_then(|v| if let Value::Int(i) = v { Some(*i) } else if let Value::Text(t) = v { t.parse().ok() } else { None }) }
    pub fn string(&self, col: &str) -> Option<String> { self.columns.get(col).and_then(|v| match v { Value::Text(t) => Some(t.clone()), Value::Int(i) => Some(i.to_string()), _ => None }) }
    pub fn columns(&self) -> Vec<String> { self.columns.keys().cloned().collect() }
}
...
/// Fluent query builder — builds SQL queries.
...
impl<'a> QueryBuilder<'a> {
    pub fn new(conn: &'a dyn Connection, table: impl Into<String>) -> Self {
        QueryBuilder { conn, table: table.into(), cols: vec!["*".into()], wheres: Vec::new(), order_bys: Vec::new(), limit: None, offset: None }
    }
    pub fn where_eq(mut self, col: impl Into<String>, val: impl Into<Value>) -> Self { self.wheres.push(WhereClause { col: col.into(), op: "=".into(), val: val.into(), bool: "and".into() }); self }
    pub fn order_by(mut self, col: impl Into<String>, dir: impl Into<String>) -> Self { self.order_bys.push((col.into(), dir.into())); self }
    ...
    fn sql(&self) -> (String, Vec<Value>) {
        ...
        if !self.wheres.is_empty() { sql.push_str(" WHERE "); for (i, w) in self.wheres.iter().enumerate() { if i > 0 { sql.push(' '); sql.push_str(&w.bool); sql.push(' '); } sql.push_str(&format!("{} {} ?", w.col, w.op)); params.push(w.val.clone()); } }
        if !self.order_bys.is_empty() { sql.push_str(" ORDER BY "); sql.push_str(&self.order_bys.iter().map(|(c, d)| format!("{} {}", c, d)).collect::<Vec<_>>().join(", ")); }
    ...
    pub fn count(&self) -> Result<u64, String> {
        let mut q = QueryBuilder::new(self.conn, &self.table); q.wheres = self.wheres.clone(); let (_s, p) = q.sql();
        let rows = self.conn.query(&format!("SELECT COUNT(*) as count FROM {}", self.table_with_where()), &p)?;
        rows.first().and_then(|r| r.int("count")).map(|c| c as u64).ok_or_else(|| "Count failed".into())
    }
    fn table_with_where(&self) -> String {
        if self.wheres.is_empty() { self.table.clone() }
        else { format!("{} WHERE {}", self.table, self.wheres.iter().map(|w| format!("{} {} ?", w.col, w.op)).collect::<Vec<_>>().join(" AND ")) }
    }
    ...
    pub fn connection_ref(&self) -> &dyn Connection { self.conn }
}
